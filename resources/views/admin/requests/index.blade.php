...
@section('content')
    <div class="admin-card mb-3">
        <div class="admin-card__body">
            <form method="GET" action="{{ route('admin.candidate-requests.index') }}" class="row g-2 align-items-end">
                <div class="col-md-5">
                    <label for="q" class="form-label">بحث</label>
                    <input type="search" id="q" name="q" class="form-control" value="{{ $filters['q'] ?? '' }}"
                           placeholder="رقم الطلب، اسم العميل، الجوال، رقم السيرة">
                </div>
                <div class="col-md-4">
    ...
    <div class="admin-card">
        <div class="admin-card__header">
            <h2 class="admin-card__title">الطلبات ({{ $requests->total() }})</h2>
        </div>
            ...
            <table class="table admin-table">
                ...
                <tbody>
                    @forelse ($requests as $request)
                        <tr>
                            <td dir="ltr">
                                <a href="{{ route('admin.candidate-requests.show', $request) }}">
